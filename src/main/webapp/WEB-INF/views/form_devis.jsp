...
<head>
<meta charset="UTF-8">
<title>Nouveau devis — Forage Web</title>
<style>
...
.card-title{font-size:1.05rem;font-weight:700;color:#0f172a;margin-bottom:18px;padding-bottom:10px;border-bottom:2px solid #e5e7eb;display:flex;align-items:center;gap:8px}
...
#lines-table th{background:#eef4ff;color:#1e3a8a;font-size:.84rem;font-weight:700;text-transform:uppercase;letter-spacing:.04em;padding:10px 10px}
...
#lines-table input,#lines-table select,#lines-table textarea{
  width:100%;border:1px solid #dbe3ef;border-radius:8px;padding:8px 10px;
  font-size:.9rem;background:#fff;outline:none;font-family:inherit;
}
...
.tfoot-total{background:#f8faff;font-weight:700;text-align:right;padding:10px 10px;border-top:2px solid #2563eb;color:#1e3a8a;font-size:1rem}
...
.info-banner{background:#eff6ff;border:1px solid #bfdbfe;border-radius:10px;padding:12px 16px;font-size:.92rem;color:#1e40af;display:flex;align-items:center;gap:10px}
</style>
</head>
<body>
...
<div class="page">
  <div class="page-header">
    <h1>📄 Nouveau devis</h1>
    <p>Un devis ne peut être créé que pour une demande acceptée.</p>
  </div>

  <div class="info-banner">
    ℹ️ Seules les demandes avec le statut <strong>Accepté</strong> apparaissent dans la liste.
  </div>
  ...
  <div class="card">
    ...
    <form id="devis-form"
          action="${pageContext.request.contextPath}/devis/enregistrer"
          method="post">
      ...
      <div class="form-grid">
        <div class="field">
          ...
          <select name="id_demande" required>
            <option value="">— Choisir une demande acceptée —</option>
            <c:forEach var="dem" items="${demandes}">
              ...
              <c:forEach var="cl" items="${clients}">
                <c:if test="${cl.idClient == dem.id_client}">
                  <c:set var="nomCl" value="${cl.nomClient}"/>
                </c:if>
              </c:forEach>
              <option value="${dem.id_demande}">
      ...
      <div class="actions">
        <a class="btn btn-secondary" href="${pageContext.request.contextPath}/devis">Annuler</a>
...
function updateTotal() {
  ...
  document.querySelectorAll('.line-row').forEach(row => {
    ...
    const mt = q * pu;
    row.querySelector('.montant').textContent = fmt(mt);
    total += mt;
  });
  document.getElementById('grand-total').textContent = fmt(total);
}

function addLine(libelle, unite, qty, pu, desc) {
  const tbody = document.getElementById('lines-body');
  const tr    = document.createElement('tr');
  tr.className = 'line-row';
  tr.innerHTML = `
    <td><input type="text"   name="libelles[]"       class="libelle" placeholder="Ex: Foration Ø150" value="${libelle||''}" required></td>
    <td><input type="text"   name="unites[]"         class="unite"   placeholder="ml" value="${unite||''}"></td>
    <td><input type="number" name="quantites[]"      class="qty"     placeholder="1"  value="${qty||1}" min="0" step="0.01"></td>
    <td><input type="number" name="prix_unitaires[]" class="pu"      placeholder="0"  value="${pu||''}" min="0" step="0.01" required></td>
    <td><textarea name="descriptions[]" class="desc" rows="2" placeholder="Détails...">${desc||''}</textarea></td>
...
// Correction noms de paramètres : on convertit [] → sans index pour Spring
document.getElementById('devis-form').addEventListener('submit', function(e) {
  e.preventDefault();
  // Renommer les champs avec index numérique
  let i = 0;
  document.querySelectorAll('.line-row').forEach(row => {
    row.querySelector('.libelle').name       = 'libelles';
    row.querySelector('.unite').name         = 'unites';
    row.querySelector('.qty').name           = 'quantites';
    row.querySelector('.pu').name            = 'prix_unitaires';
    row.querySelector('.desc').name          = 'descriptions';
    i++;
  });
  this.submit();
});
